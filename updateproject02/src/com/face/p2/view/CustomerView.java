...
//		CustomerList customerList = new CustomerList();
	...
	private void listAllCustomers() {
		...
		if (total == 0) {//total 为0
			...
			ArrayList<Customer> custs = customerList.getAllCustomers();// 返回所有客户对象在一个新的数组中 防止有空值

			Iterator<Customer> iterator = custs.iterator();
			while (iterator.hasNext()){
				Customer cust = iterator.next();

						System.out.println(custs.size() + "\t" + cust.getName() + "\t" + cust.getGender() + "\t" + cust.getAge()
